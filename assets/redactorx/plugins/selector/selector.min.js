RedactorX.add("plugin", "selector", {
  ...
  start: function () {
    ...
    this.app.toolbar.add("selector", t),
      (t.position = { before: "duplicate" }),
      this.app.control.add("selector", t);
  },
  popup: function (t, s) {
    this.app.popup.create("selector", this.popups.base);
    for (
      var e = this.app.block.get().getBlock(),
        a = e.attr("class"),
        l = e.attr("id"),
        o = a.split(" "),
        p = ((a = ""), 0);
      ...
      -1 === o[p].search(this.prefix + "-")
        ? (a += o[p] + " ")
        : this.editorClasses.push(o[p]);
    this.app.popup.setData({ id: l, classname: a.trim() }),
      this.app.popup.open({ button: s, focus: "classname" });
  },
  save: function () {
    this.app.popup.close();
    var t = this.app.block.get().getBlock(),
      s = this.app.popup.getData();
    "" === s.id ? t.removeAttr("id") : t.attr("id", s.id),
      0 < this.editorClasses.length &&
        (s.classname = s.classname + " " + this.editorClasses.join(" ")),
      t.attr("class", s.classname),
      (this.editorClasses = []);
  },
});
