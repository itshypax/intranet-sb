RedactorX.add("plugin", "clips", {
  translations: { en: { clips: { clips: "Clips" } } },
  defaults: {
  ...
  popup: function (t, i) {
    ...
    for (var s in this.opts.clips.items)
      p[s] = { title: this.opts.clips.items[s].title, command: "clips.insert" };
    this.app.popup.create("clips", { items: p }),
      this.app.popup.open({ button: i });
  },
  insert: function (t, i, p) {
    this.app.popup.close();
    var s = this.opts.clips.items[p].html;
    this.app.editor.insertContent({ html: s });
  },
});
