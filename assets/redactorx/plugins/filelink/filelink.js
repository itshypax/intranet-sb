RedactorX.add("plugin", "filelink", {
  ...
  popup: function (params, button) {
    ...
    if (this.opts.filelink.upload) {
      ...
      var popup = this.app.popup.create(
        "file",
        this._buildPopupUpload(this.popups.create)
      );
    ...
    if (this.opts.filelink.select) {
      var spopup = this.app.popup[choosetype](
        "file-select",
        this.popups.choose
      );
  ...
  _truncateUrl: function (url) {
    return url.search(/^http/) !== -1 && url.length > 20
      ? url.substring(0, 20) + "..."
      : url;
  },
});
