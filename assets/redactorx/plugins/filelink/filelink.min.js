RedactorX.add("plugin", "filelink", {
  ...
  popup: function (t, i) {
    ...
    if (this.opts.filelink.upload) {
      e = "add";
      var s = this.app.selection.getText();
      this.app.popup
        .create("file", this._buildPopupUpload(this.popups.create))
        .setData({ title: s });
    }
    if (this.opts.filelink.select) {
      var a = this.app.popup[e]("file-select", this.popups.choose);
      (this.$sbox = a.getBody()),
        "string" == typeof this.opts.filelink.select
          ? this.ajax.get({
    ...
    this.app.popup.open({ button: i });
  },
  insertByUpload: function (t) {
    this.app.popup.close();
    var i = this.app.popup.getData();
    this._insert(t, i.title);
  },
  insertBySelect: function (t) {
    t.preventDefault(), this.app.popup.close();
    var i = this.app.selection.getText(),
      e = this.dom(t.target).closest("." + this.prefix + "-popup-list-item"),
      s = JSON.parse(decodeURI(e.attr("data-params")));
    this._insert({ file: s }, i, !0);
  },
  error: function (t) {
  ...
  getStates: function () {
    var t = this._findFiles();
    for (var i in this.dataStates) {
      var e = this.dataStates[i],
        s = t.is('[data-file="' + e.id + '"]');
      this._setFileState(e.id, s);
    }
  ...
  _insert: function (t, i, e) {
    for (var s in t) {
      var a = this._createFileAndStore(t[s], i),
        l = e ? "select" : "upload";
      this.app.broadcast("file." + l, { response: t, $el: a });
      break;
    }
  },
  ...
  _setFileState: function (t, i) {
    this.dataStates[t].status = i;
  },
  _parseList: function (t) {
    var i = this.dom("<ul>");
    for (var e in (i.addClass(this.prefix + "-popup-list"), t)) {
      var s = t[e];
      if ("object" == typeof s) {
        var a = this.dom("<li>"),
          l = this.dom("<span>");
        a.append(l),
          l.addClass(this.prefix + "-popup-list-item"),
          l.attr("data-params", encodeURI(JSON.stringify(s))),
          l.text(s.title || s.name);
        var p = this.dom("<span>");
        p.addClass(this.prefix + "-popup-list-aside"),
          p.text(s.name),
          a.append(p);
        var o = this.dom("<span>");
        o.addClass(this.prefix + "-popup-list-aside"),
          o.text("(" + s.size + ")"),
          a.append(o),
          a.on("click", this.insertBySelect.bind(this)),
          i.append(a);
      }
    }
    this.$sbox.append(i);
  },
  _createFileAndStore: function (t, i) {
    var e = this.app.inline.set({ tag: "a", caret: "after" }),
      s = this.dom(e);
    return (
      s.attr("href", t.url),
      s.attr("data-file", t.id ? t.id : this.app.utils.getRandomId()),
      s.attr("data-name", t.name),
      this.opts.filelink.classname && s.addClass(this.opts.filelink.classname),
      (i = i && "" !== i ? i : this._truncateUrl(t.name)),
      s.html(i),
      s
    );
  },
  _truncateUrl: function (t) {
    return -1 !== t.search(/^http/) && 20 < t.length
      ? t.substring(0, 20) + "..."
      : t;
  },
});
